...
def test_stable_source_id_dedupes_in_silver(bronze_uri: str, checkpoint_path: Path, run_id: str):
    """Bronze intentionally makes the ghost retry's duplicate visible (see
    above). Silver is where the stable source_event_id resolves it -- the
    reference design from article 2 preserves enough source identity in
    Bronze that retries are visible and repairable, and lets Silver apply
    the technical uniqueness rule.
    """
    source = SyntheticOrdersAPI(pages=3, page_size=5, start_page=41)
    cp = Checkpoint(checkpoint_path)
    ...
    ingest_window(source=source, table_uri=bronze_uri, run_id=run_id, checkpoint=cp)  # the retry

    bronze_rows = DeltaTable(bronze_uri).to_pyarrow_table().num_rows
    assert bronze_rows == 20  # the duplicate is visible in Bronze

    silver = prepare_orders(bronze_uri=bronze_uri)
    assert silver.num_rows == 15  # Silver resolves the duplicate by source_event_id (-> order_id)
    order_ids = silver.column("order_id").to_pylist()
